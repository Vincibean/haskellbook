...
instance (Show smas, Show1 m) => Show1 (EitherT smas m) where
  liftShowsPrec sp sl d (EitherT m) = showsUnaryWith (liftShowsPrec sp' sl')
                                                     "EitherT"
                                                     d
                                                     m
   where
...
--   fmap f (StateT' smas) =
--     StateT' $ \s -> let mas = smas s in (\(a, s') -> (f a, s')) <$> mas

instance (Functor m) => Functor (StateT' s m) where
  fmap f = StateT' . (fmap . fmap) (first f) . runStateT'

-- 2. Write the Applicative instance for StateT
