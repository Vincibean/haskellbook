...
spec = do
  describe "EitherT" $ do
    it "1. Write the Functor instance for EitherT"
      $ let trigger =
              undefined :: EitherT [Bool] Identity (String, String, Int)
    ...
    it "4. Write the swapEitherT helper function for EitherT"
      $ property
      $ \e ->
          let swapped :: EitherT [Bool] Identity (String, String, Int)
  ...
  describe "Write the code" $ do
    it "1. 2. rDec" $ fmap (runReader rDec) [1 .. 10] `shouldBe` [0 .. 9]
    it "3. 4. rShow"
